...
class Team extends Model {
	...
	protected $fillable = array('team_owner_id','sports_id','organization_id','name','address','city_id','state_id','country_id','zip','description','city','state','country','location','team_available','player_available','gender','team_level','logo');
	...
	public function searchResults($req_params)
	{
		...
		if(trim($req_params['sport']) != ''){
			$query = $query->whereIn('sports_id',explode(",",$req_params['sport']));				
						
		}
		if(trim($req_params['gender']) != ''){
			$query = $query->whereIn('gender',explode(",",$req_params['gender']));						
		}
		...
		if(trim($req_params['joinavialability']) != ''){
			$query = $query->whereIn('player_available',explode(",",$req_params['joinavialability']));						
		}
