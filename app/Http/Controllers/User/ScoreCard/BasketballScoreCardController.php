...
use App\Model\BasketballPlayerMatchScore;
...
class BasketballScoreCardController extends parentScoreCardController
{
 
 public function basketballScoreCard($match_data,$sportsDetails=[],$tournamentDetails=[],$is_from_view=0)
    {
        $loginUserId = '';
        $loginUserRole = '';
        ...
        $team_a_basketball_scores_array = array();
        if(count($team_a_basketball_scores)>0)
        {
            $team_a_basketball_scores_array = $team_a_basketball_scores->toArray();
        }

        //get Basketball scores for team b
        $team_b_basketball_scores = BasketballPlayerMatchwiseStats::select()->where('match_id',$match_data[0]['id'])->where('team_id',$team_b_id)->get();
        $team_b_basketball_scores_array = array();
        if(count($team_b_basketball_scores)>0)
        {
            $team_b_basketball_scores_array = $team_b_basketball_scores->toArray();
        }
        ...
        $team_a_players_stat=BasketballPlayerMatchwiseStats::whereMatchId($match_id)->whereTeamId($team_a_id)->get();
        $team_b_players_stat=BasketballPlayerMatchwiseStats::whereMatchId($match_id)->whereTeamId($team_b_id)->get();
    ...
    public function confirmSquad(){
        ...
        foreach($team_a_playing_players as $p){
            ...
            $this->insertBasketballScore($p, $tournament_id, $match_id, $team_a_id,$player_name, $team_a_name,'P');
            $players_a_details['player_'.$p]=$default_player_details;
        }
        foreach($team_a_substitute_players as $p){
            $player_name=User::find($p)->name;
            $this->insertBasketballScore($p, $tournament_id, $match_id, $team_a_id,$player_name, $team_a_name,'S');
            $players_a_details['player_'.$p]=$default_player_details; 
        }
        foreach($team_b_playing_players as $p){
            ...
            $player_name=User::find($p)->name;
             $default_player_details['playing_status']=1;
            $this->insertBasketballScore($p, $tournament_id, $match_id, $team_b_id,$player_name, $team_b_name,'P');
        	$players_b_details['player_'.$p]=$default_player_details;
        }
        foreach($team_b_substitute_players as $p){          
            $player_name=User::find($p)->name;
            $this->insertBasketballScore($p, $tournament_id, $match_id, $team_b_id,$player_name, $team_b_name,'S');
          	$players_b_details['player_'.$p]=$default_player_details;     
        }
      ...
      public function insertBasketBallScore($user_id,$tournament_id,$match_id,$team_id,$player_name,$team_name,$playing_status='S')
    {
        $basketball_model = new BasketballPlayerMatchwiseStats();
        $basketball_model->user_id          = $user_id;
        $basketball_model->tournament_id    = $tournament_id;
        $basketball_model->match_id         = $match_id;
        $basketball_model->team_id          = $team_id;
        $basketball_model->player_name      = $player_name;
        $basketball_model->team_name        = $team_name;
        $basketball_model->playing_status   = $playing_status;
        $basketball_model->save();
    }
    ...
    public function manualScoring(ObjectRequest $request){
    		...
    		$players_stats=BasketballPlayerMatchwiseStats::whereMatchId($match_id)->get();
     ...
     public function basketballStoreRecord(ObjectRequest $Objrequest){
...
//                                Helper::printQueries();
                    ...
                    $sportName = Sport::where('id',$matchScheduleDetails['sports_id'])->pluck('sports_name');
                        ...
                        //notification ocde
                        Helper::sendEmailPlayers($matchScheduleDetails, 'Basketball');      
    	...
    	public function basketballStatistics($user_id)
	{
		...
		public function basketballSwapPlayers(){
		$request=Request::all();
		$match_id=$request['match_id'];
		$team_id=$request['team_id'];
		$time_substituted=$request['time_substituted'];
		$basketball_model=BasketballPlayerMatchwiseStats::whereMatchId($match_id)->whereTeamId($team_id)->get();

		foreach ($basketball_model as $sm ){
			$sm_id=$sm->id;
	 ...
	 public function basketballSaveRecord(){
            $request=Request::all();
            $match_id=$request['match_id'];
        ...
        $basketball_players=BasketballPlayerMatchwiseStats::whereMatchId($match_id)->first();
       ...
       $basketball_model= BasketballPlayerMatchwiseStats::find($player_stat_id);
       $team_id=$basketball_model->team_id;
      ...
      $match_details->{$team_id}->players->{'player_'.$basketball_model->user_id}->total_points=$basketball_model->total_points;
                	...
                	case 'points_3':
           $basketball_model->points_3++;
           $basketball_model->{$quarter}+=3;
          $match_details->{$team_id}->players->{'player_'.$basketball_model->user_id}->total_points+=3;
            $match_details->{$team_id}->players->{'player_'.$basketball_model->user_id}->points_3++;;
          $match_details->{$team_id}->players->{'player_'.$basketball_model->user_id}->{$quarter}->points=$basketball_model->points_3;
      ...
      $match_details->{$team_id}->players->{'player_'.$basketball_model->user_id}->total_points=$basketball_model->total_points;
                	...
                	case 'fouls':
               if($basketball_model->fouls>=$max_fouls){
               	   $match_details->{$team_id}->players->{'player_'.$user_id}->dismissed=1;
               	   $basketball_model->playing_status='S';
               }

          $basketball_model->fouls++;
        ...
        //$this->updateBasketballScore($user_id,$match_id,$team_id,$player_name,$points_1, $points_2, $points_3, $total_points, $fouls);
        $this->basketballStatistics($user_id);          
    ...
    public function updateBasketballScore($user_id,$match_id,$team_id,$player_name,$points_1,$points_2,$points_3, $total_points, $fouls)
	{
		$player_stat=BasketballPlayerMatchwiseStats::where('user_id',$user_id)->where('match_id',$match_id)->where('team_id',$team_id)->update(['user_id'=>$user_id,
			'points_1'=>$points_1,
			'points_2'=>$points_2,
			'points_3'=>$points_3,
			'total_points'=>$total_points,
