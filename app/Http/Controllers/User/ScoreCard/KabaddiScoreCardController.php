 ...
 public function kabaddiScoreCard($match_data,$sportsDetails=[],$tournamentDetails=[],$is_from_view=0)
    {
        ...
        {
            $team_a_kabaddi_scores_array = $team_a_kabaddi_scores->toArray();
        }
        ...
        {
            $team_b_kabaddi_scores_array = $team_b_kabaddi_scores->toArray();
        }
      ...
      public function insertkabaddiScore($user_id,$tournament_id,$match_id,$team_id,$player_name,$team_name,$playing_status='S')
    {
        ...
        $kabaddi_model->team_id          = $team_id;
        $kabaddi_model->player_name      = $player_name;
        $kabaddi_model->team_name        = $team_name;
        $kabaddi_model->playing_status   = $playing_status;
        $kabaddi_model->save();
    }
...
//                                Helper::printQueries();
                    ...
                    $sportName = Sport::where('id',$matchScheduleDetails['sports_id'])->pluck('sports_name');
                        ...
                        //notification ocde
                        Helper::sendEmailPlayers($matchScheduleDetails, 'kabaddi');      
        ...
        public function kabaddiStatistics($user_id)
    {
        ...
        {
            $match_count = (!empty($kabaddi_details[0]['match_count']))?$kabaddi_details[0]['match_count']:0;
        ...
        public function kabaddiSwapPlayers(){
        $request=Request::all();
        $match_id=$request['match_id'];
