...
class ThrowballscoreCardController extends parentScoreCardController
{
 ...
 public function throwballScoreCard($match_data,$sportsDetails=[],$tournamentDetails=[],$is_from_view=0)
    ...
    $throwball_b_score=throwballScore::whereMatchId($match_data[0]['id'])->whereTeamId($match_data[0]['b_id'])->first();
        ...
        if($is_from_view==1 || (!empty($score_status_array['added_by']) && $score_status_array['added_by']!=$loginUserId && $match_data[0]['scoring_status']!='rejected') || $match_data[0]['match_status']=='completed' || $match_data[0]['scoring_status']=='approval_pending' || $match_data[0]['scoring_status']=='approved' || !$isValidUser)//throwball score view only
        {
