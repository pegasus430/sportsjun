...
use App\Model\WaterpoloPlayerMatchScore;
use App\Model\WaterpoloStatistic;
use App\Model\Photo;
use App\User;
use DB;
use Carbon\Carbon;
...
class WaterPoloScorecardController extends parentScoreCardController
{
 
 public function waterpoloScoreCard($match_data,$sportsDetails=[],$tournamentDetails=[],$is_from_view=0)
    {
        $loginUserId = '';
        $loginUserRole = '';
        ...
        $match_players = waterpoloPlayerMatchwiseStats::whereMatchId($match_id)->get(['user_id'])->toArray();

        //get waterpolo scores for team a
        $team_a_waterpolo_scores = waterpoloPlayerMatchwiseStats::select()->where('match_id',$match_data[0]['id'])->where('team_id',$team_a_id)->get();

        $team_a_waterpolo_scores_array = array();
        if(count($team_a_waterpolo_scores)>0)
        {
            $team_a_waterpolo_scores_array = $team_a_waterpolo_scores->toArray();
        }

        //get waterpolo scores for team b
        $team_b_waterpolo_scores = waterpoloPlayerMatchwiseStats::select()->where('match_id',$match_data[0]['id'])->where('team_id',$team_b_id)->get();
        $team_b_waterpolo_scores_array = array();
        if(count($team_b_waterpolo_scores)>0)
        {
            $team_b_waterpolo_scores_array = $team_b_waterpolo_scores->toArray();
        }
        ...
        $team_a_players_stat=waterpoloPlayerMatchwiseStats::whereMatchId($match_id)->whereTeamId($team_a_id)->get();
        $team_b_players_stat=waterpoloPlayerMatchwiseStats::whereMatchId($match_id)->whereTeamId($team_b_id)->get();
        ...
        if(($is_from_view==1 || (!empty($score_status_array['added_by']) && $score_status_array['added_by']!=$loginUserId && $match_data[0]['scoring_status']!='rejected') || $match_data[0]['match_status']=='completed' || $match_data[0]['scoring_status']=='approval_pending' || $match_data[0]['scoring_status']=='approved' || !$isValidUser) && !$isAdminEdit)//waterpolo score view only
        {
    ...
    public function confirmSquad(){
        ...
        foreach($team_a_playing_players as $p){
            ...
            $this->insertwaterpoloScore($p, $tournament_id, $match_id, $team_a_id,$player_name, $team_a_name,'P');
            $players_a_details['player_'.$p]=$default_player_details;
        }
        foreach($team_a_substitute_players as $p){
            $player_name=User::find($p)->name;
            $this->insertwaterpoloScore($p, $tournament_id, $match_id, $team_a_id,$player_name, $team_a_name,'S');
            $players_a_details['player_'.$p]=$default_player_details; 
        }
        foreach($team_b_playing_players as $p){
            ...
            $player_name=User::find($p)->name;
             $default_player_details['playing_status']=1;
            $this->insertwaterpoloScore($p, $tournament_id, $match_id, $team_b_id,$player_name, $team_b_name,'P');
            $players_b_details['player_'.$p]=$default_player_details;
        }
        foreach($team_b_substitute_players as $p){          
            $player_name=User::find($p)->name;
            $this->insertwaterpoloScore($p, $tournament_id, $match_id, $team_b_id,$player_name, $team_b_name,'S');
            $players_b_details['player_'.$p]=$default_player_details;     
        }
      ...
      public function insertwaterpoloScore($user_id,$tournament_id,$match_id,$team_id,$player_name,$team_name,$playing_status='S')
    {
        $waterpolo_model = new waterpoloPlayerMatchwiseStats();
        $waterpolo_model->user_id          = $user_id;
        $waterpolo_model->tournament_id    = $tournament_id;
        $waterpolo_model->match_id         = $match_id;
        $waterpolo_model->team_id          = $team_id;
        $waterpolo_model->player_name      = $player_name;
        $waterpolo_model->team_name        = $team_name;
        $waterpolo_model->playing_status   = $playing_status;
        $waterpolo_model->save();
    }
    ...
    public function manualScoring(ObjectRequest $request){
            ...
            $players_stats=waterpoloPlayerMatchwiseStats::whereMatchId($match_id)->get();
     ...
     public function waterpoloStoreRecord(ObjectRequest $Objrequest){
       ...
       $request=Request::all();
        ...
        $waterpolo_player=waterpoloPlayerMatchwiseStats::whereMatchId($match_id)->first();
...
//                                Helper::printQueries();
                    ...
                    $sportName = Sport::where('id',$matchScheduleDetails['sports_id'])->pluck('sports_name');
                        ...
                        //notification ocde
                        Helper::sendEmailPlayers($matchScheduleDetails, 'waterpolo');      
        ...
        public function waterpoloStatistics($user_id)
    {
        //check already player has record or not
        $user_waterpolo_details = waterpoloStatistic::select()->where('user_id',$user_id)->get();

        $waterpolo_details = waterpoloPlayerMatchwiseStats::selectRaw('count(match_id) as match_count')->selectRaw('sum(points_1) as points_1')->selectRaw('sum(points_2) as points_2')->selectRaw('sum(points_3) as points_3')->selectRaw('sum(total_points) as total_points')->selectRaw('sum(fouls) as fouls')->where('user_id',$user_id)->groupBy('user_id')->get();



        $points_1 = (!empty($waterpolo_details[0]['points_1']))?$waterpolo_details[0]['points_1']:0;
        $points_2 = (!empty($waterpolo_details[0]['points_2']))?$waterpolo_details[0]['points_2']:0;
        $points_3 = (!empty($waterpolo_details[0]['points_3']))?$waterpolo_details[0]['points_3']:0;
        $fouls = (!empty($waterpolo_details[0]['fouls']))?$waterpolo_details[0]['fouls']:0;
        $total_points = (!empty($waterpolo_details[0]['total_points']))?$waterpolo_details[0]['total_points']:0;

        if(count($user_waterpolo_details)>0)
        {
            $match_count = (!empty($waterpolo_details[0]['match_count']))?$waterpolo_details[0]['match_count']:0;

            waterpoloStatistic::where('user_id',$user_id)
        ...
        {
            $waterpolo_statistic = new waterpoloStatistic();
            $waterpolo_statistic->user_id = $user_id;
            $waterpolo_statistic->matches = 1;
            $waterpolo_statistic->{'points_1'} = $points_1;
            $waterpolo_statistic->{'points_2'} = $points_2;
            $waterpolo_statistic->{'points_3'} = $points_3;
            $waterpolo_statistic->fouls = $fouls;
            $waterpolo_statistic->total_points = $total_points;
            $waterpolo_statistic->save();
        }
    }

        public function waterpoloSwapPlayers(){
        $request=Request::all();
        $match_id=$request['match_id'];
        $team_id=$request['team_id'];
        $time_substituted=$request['time_substituted'];
        $waterpolo_model=waterpoloPlayerMatchwiseStats::whereMatchId($match_id)->whereTeamId($team_id)->get();

        foreach ($waterpolo_model as $sm ){
            $sm_id=$sm->id;
        ...
        return $waterpolo_model;

    }

     public function waterpoloSaveRecord(){
            $request=Request::all();
            $match_id=$request['match_id'];
        ...
        $waterpolo_players=waterpoloPlayerMatchwiseStats::whereMatchId($match_id)->first();
       ...
       $waterpolo_model= waterpoloPlayerMatchwiseStats::find($player_stat_id);
       $team_id=$waterpolo_model->team_id;
          ...
          $waterpolo_model->points_1++;
          $waterpolo_model->{$quarter}++;
          
          $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->points_1++;
          $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->{$quarter}->points_1++;
          $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->{$quarter}->total_points=$waterpolo_model->{$quarter};
          
          $waterpolo_model->total_points=$this->getTotal($waterpolo_model);

      $match_details->{$team_id}->total_points++;
      $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->total_points=$waterpolo_model->total_points;

                        break;

                    case 'points_2':
           $waterpolo_model->points_2++;
           $waterpolo_model->{$quarter}+=2;

    
       $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->points_2++;
       $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->{$quarter}->points_2++;
       $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->{$quarter}->total_points=$waterpolo_model->{$quarter};
      ...
      $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->total_points=$waterpolo_model->total_points;
                    ...
                    case 'points_3':
           $waterpolo_model->points_3++;
           $waterpolo_model->{$quarter}+=3;
          $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->total_points+=3;
            $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->points_3++;;
          $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->{$quarter}->points=$waterpolo_model->points_3;
      ...
      $match_details->{$team_id}->players->{'player_'.$waterpolo_model->user_id}->total_points=$waterpolo_model->total_points;
                    ...
                    case 'fouls':
               if($waterpolo_model->fouls>=$max_fouls){
                   $match_details->{$team_id}->players->{'player_'.$user_id}->dismissed=1;
                   $waterpolo_model->playing_status='S';
               }

          $waterpolo_model->fouls++;
        ...
        //$this->updatewaterpoloScore($user_id,$match_id,$team_id,$player_name,$points_1, $points_2, $points_3, $total_points, $fouls);
        $this->waterpoloStatistics($user_id);          
    ...
    public function updatewaterpoloScore($user_id,$match_id,$team_id,$player_name,$points_1,$points_2,$points_3, $total_points, $fouls)
    {
        $player_stat=waterpoloPlayerMatchwiseStats::where('user_id',$user_id)->where('match_id',$match_id)->where('team_id',$team_id)->update(['user_id'=>$user_id,
            'points_1'=>$points_1,
            'points_2'=>$points_2,
            'points_3'=>$points_3,
            'total_points'=>$total_points,
        ...
        //waterpoloStatistic::where('user_id',$user_id)->update(['yellow_cards'=>$yellow_card_count,'red_cards'=>$red_card_count,'goals_scored'=>$goal_count]);
    }
    //
