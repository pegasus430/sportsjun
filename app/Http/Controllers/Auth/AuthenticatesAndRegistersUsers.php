...
trait AuthenticatesAndRegistersUsers {
    ...
    public function postRegister(Request $request) {
        $this->validate($request, [
           'firstname' => 'required|max:255',
			'lastname' => 'required|max:255', 'email' => 'required|email|max:255|unique:users,email', 'password' => 'required|confirmed|min:6','captcha' => 'required|captcha'
		...
		if(count($userDetails) > 0){
			if($userDetails->is_verified == 0){
				if ($this->auth->attempt($credentials, $request->has('remember'))) {
					$profilePhoto = Photo::where(['user_id' => Auth::user()->id, 'is_album_cover' => 1, 'imageable_type' => config('constants.PHOTO.USER_PHOTO')])->get(['url']);
    ...
    protected function getFailedLoginMessage() {
	 ...
	 protected function getFailedVerifyMessage($email) {
        return 'Your email is not verified yet. <a href="/resendverifyemail/'.$email.'">Resend</a> verification email';
    }
    ...
    public function create(array $data) {
		...
		$view = 'emails.welcome';
		$mail_data = array('view'=>$view,'subject'=>$subject,'to_email_id'=>$to_email_id,'view_data'=>$view_data,'flag'=>'user','send_flag'=>1,'verification_key'=>$verification_key);
