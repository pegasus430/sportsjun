...
class User extends Model implements AuthenticatableContract, AuthorizableContract, CanResetPasswordContract {
     ...
     * @var array
     */
    protected $fillable = ['firstname', 'lastname', 'name', 'email', 'password', 'dob', 'gender', 'location', 'address', 'city_id', 'city', 'state_id', 'state', 'country_id', 'country', 'zip', 'contact_number', 'about', 'newsletter', 'profile_updated', 'role','verification_key','is_verified','isactive'];
    ...
    public static function validate($data, $context) {
        ...
        if ($context == 'edit') {
            ...
            $messages = [
                ...
                'contact_number.numeric' => trans('validation.numeric', ['attribute' => 'contact number']),
	...
	public function searchResults($req_params)
	{
		...
		$query = $this->search($req_params['search_by'])->join('user_statistics', 'user_statistics.user_id', '=', 'users.id');
		...
		if(trim($req_params['sport']) != ''){
			...
			foreach($sports_arr as $sport){
				$sport = ",".$sport.",";
				if($i == 0)
					$query = $query->where('user_statistics.following_sports', 'LIKE', "%$sport%");
				else
					$query = $query->orwhere('user_statistics.following_sports', 'LIKE', "%$sport%");
				
				$i++;
			}			
		}
		if(trim($req_params['gender']) != ''){
			$query = $query->whereIn('users.gender',explode(",",$req_params['gender']));						
		}
		...
		if(trim($req_params['joinavialability']) == 1){
			...
			foreach($sports_arr as $sport){
				$sport = ",".$sport.",";
				if($i == 0)
					$query = $query->where('user_statistics.allowed_sports', 'LIKE', "%$sport%");
				else
					$query = $query->orwhere('user_statistics.allowed_sports', 'LIKE', "%$sport%");
		...
		$result = $query->limit($limit)->offset($offset)->orderBy('users.updated_at', 'desc')->get();
