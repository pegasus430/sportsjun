        ...
        <div class="col-md-6">
            <span class="switch-head">{{trans('message.sports.availableforteam_view')}}</span>
                @if(Auth::user()->id == $userId)
                    <input class="switch-class" type="checkbox" name="chk_available" id="chk_available" sportid="{{ !empty($sportsId)?$sportsId:0 }}" userid="{{ Auth::user()->id }}" {{ in_array($sportsId, $existingAllowedSportsArray)?'checked': '' }}>
                @else
    ...
    @if($sportDetails->sports_type=='both' || $sportDetails->sports_type=='player')
        <div class="col-md-6">
            <span class="switch-head">{{trans('message.sports.allowfollowerstofollow_view')}}</span>
                @if(Auth::user()->id == $userId)
                    <input class="switch-class" type="checkbox"  name="chk_follow"  id="chk_follow" sportid="{{ !empty($sportsId)?$sportsId:0 }}" userid="{{ Auth::user()->id }}" {{ in_array($sportsId, $existingAllowedMatchesArray)?'checked': '' }}> 
                @else
