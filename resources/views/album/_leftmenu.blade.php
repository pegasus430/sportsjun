<div class="col-sm-2" id="sidebar-left">
    <div class="row">
        ...
        <!--<li><a class="sidemenu_1" href="{{ url('user/info') }}">Info</a></li>-->
            <li><a class="sidemenu_2" href="{{ url('showsportprofile',$userId) }} {{Request::has('from_org')?'?from_org=true':''}}"><span class="ico ico-members"></span> Sports Profile</a></li>
            <li><a class="sidemenu_3" href="{{ url('team/teams',$userId) }} {{Request::has('from_org')?'?from_org=true':''}}"><span class="ico ico-teams"></span> Teams</a></li>
            <li><a class="sidemenu_4" href="{{ url('user/album/show').'/user/'.$userId }} {{Request::has('from_org')?'?from_org=true':''}}"><span class="ico ico-media-gallery"></span> Media Gallery</a></li>
            @if(isset($userId) && ($userId == (isset(Auth::user()->id)?Auth::user()->id:0)))
            ...
            @endif
            <li><a class="sidemenu_6" href="{{ url('/myschedule',[$userId]) }} {{Request::has('from_org')?'?from_org=true':''}}"><span class="ico ico-schedule"></span> {{($userId == (isset(Auth::user()->id)?Auth::user()->id:0))?'My Schedule':'Schedule'}}</a></li>
            <li><a class="sidemenu_7" href="{{ url('/following',[$userId]) }} {{Request::has('from_org')?'?from_org=true':''}}"><span class="ico ico-followers"></span> Following</a></li>
        </ul>
    </div>
</div>