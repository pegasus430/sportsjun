            ...
            <li class="col-xs-6 col-sm-4 col-md-3">
                <div class="thumbnail">
                    <div class="ImageWrapper userpro-img">
                        ...
                        !!}
                        <a href="{{url('/createphoto/'.$user_profile_album[0]['id'].'/'. $user_profile_album[0]['user_id'].'/'.'1')}}">User
                            Profile Album ( {{ count($user_profile_album) }} )</a>
                    <!--<div class="actions">
			   <ul>
                ...
                <div class="thumbnail">
                    <div class="ImageWrapper userpro-img">
                        ...
                        @if (!(isset($is_widget) && $is_widget))
                            <a href="{{url('/formgallery/'.$action_id.'/'.'formorganization')}}">Form
                                gallery( {{ count($organization_profile_albums) }} )</a>
                        @else
                            <a href="{{route('widget.formgallery',['id'=>$action_id,'action'=>'formorganization'])}}">Form
                                gallery( {{ count($organization_profile_albums) }} )</a>
                        @endif
                    </div>
                </div>
            ...
            <li class="col-xs-6 col-sm-4 col-md-3">
                <div class="thumbnail">
                    <div class="ImageWrapper userpro-img">
                        ...
                        !!}
                        <a href="{{url('/formgallery/'.$action_id.'/'.'formtournaments')}}">Form
                            gallery( {{ count($tournament_profile_albums) }} )</a>
                        ...
                        <div class="thumbnail">
                            <div class="ImageWrapper">
                                ...
                                <div class="create-album">
                                    <button type="button" class="button btn-primary launch-modal create_album"
                                            data-toggle="modal" data-target=".bs-example-modal-sm"><i
                                                class="fa fa-plus"></i> CREATE ALBUM
                                    </button>
                                </div>
                ...
                @if(isset($photo_arrayy[$album['id']][0]['url']))
                    <li class="col-xs-6 col-sm-4 col-md-3 remove_li_{{$album['id']}}">
                        <div class="thumbnail">
                            <div class="ImageWrapper">
                                @if (!(isset($is_widget) && $is_widget))
                                    <a href="{{url($check_user.'/createphoto/'.$album['id'].'/'.$album['user_id'].'/0/'.$action.'/'.$action_id)}}">{!! Helper::Images($photo_arrayy[$album['id']][0]['url'],'gallery/'.$photo_arrayy[$album['id']][0]['imageable_type'],array('height'=>200,'width'=>200,
		         'id'=>$photo_arrayy[$album['id']][0]['imageable_id']) )!!}</a>
                                @else
                                    <a href="{{ route('widget.createphoto',['album_id'=>$album['id'],'user_id'=>$album['user_id'],'is_user'=>0,'action'=>$action,'team_id'=>$action_id]) }}">{!! Helper::Images($photo_arrayy[$album['id']][0]['url'],'gallery/'.$photo_arrayy[$album['id']][0]['imageable_type'],array('height'=>200,'width'=>200,
		         'id'=>$photo_arrayy[$album['id']][0]['imageable_id']) )!!}</a>
                                @endif
                                ...
                                @if (!(isset($is_widget) && $is_widget))
                                    @if($action=='team' )
                                        ...
                                        @if($album['accessflag']=='yes' )
                                            <div class="actions">
                                                <ul>
                                                    <li><a href="#" data-toggle="modal" data-pid="{{ $album['id'] }}"
                                                           onclick="editalbum({{$album['id'] }})"
                                                           class="launch-modal editalbum icon-edit"><i
                                                                    class="fa fa-pencil"></i></a></li>
                                                    <li><a href="javascript:void(0);"
                                                           class="removeAlbumPhoto icon-delete"
                                                           data-pid="{{ $album['id'] }}" data-page="{{'album'}}"
                                                           title="Delete"><i class="fa fa-remove"></i></a></li>
                                                </ul>
                                            </div>
                                        @endif
                                    @endif
                                    ...
                                    @if($action=='organization' && $album['user_id']==$loginUserId)
                                        ...
                                        <div class="actions">
                                            <ul>
                                                ...
                                                <li><a href="javascript:void(0);" class="removeAlbumPhoto icon-delete"
                                                       data-pid="{{ $album['id'] }}" data-page="{{'album'}}"
                                                       title="Delete"><i
                        ...
                        @if (!(isset($is_widget) && $is_widget))
                            <div class="caption">
            	<span class="lead">
                	<a id="editalbum_{{ $album['id'] }}"
                       href="{{url('/createphoto/'.$album['id'].'/'.$album['user_id'].'/0/'.$action.'/'.$action_id)}}">{{ $album['title'] }}</a> <span>( {{ $album_photo_count[$album['id']] }}
                        )</span>
                </span>
                            </div>
                        @endif
                ...
                @else
                    <li class="col-xs-6 col-sm-4 col-md-3 remove_li_{{$album['id']}}">
                        <div class="thumbnail">
                            <div class="ImageWrapper">
                                @if (!(isset($is_widget) && $is_widget))
                                    <a href="{{url($check_user.'/createphoto/'.$album['id'].'/'.$album['user_id'].'/0/'.$action.'/'.$action_id)}}">{!! Helper::Images('market_place_default.png','marketplace',array('height'=>200,'width'=>200) )!!}</a>
                                @else
                                    <a href="{{ route('widget.createphoto',['album_id'=>$album['id'],'user_id'=>$album['user_id'],'is_user'=>0,'action'=>$action,'team_id'=>$action_id]) }}">{!! Helper::Images('market_place_default.png','marketplace',array('height'=>200,'width'=>200) )!!}</a>
                                @endif
                                @if (!(isset($is_widget) && $is_widget))
                                    @if($action=='team'  )
                                        @if($album['accessflag']=='yes')
                	...
                	<span class="lead">
                    	...
                    	<a id="editalbum_{{ $album['id'] }}"
                           href="{{url($check_user.'/createphoto/'.$album['id'].'/'.$album['user_id'].'/0/'.$action.'/'.$action_id)}}">{{ $album['title'] }}</a><span> ( {{ $album_photo_count[$album['id']] }}
                            )</span>
                    </span>
                        </div>
                            @endif
