...
input:read-only { 
  ...
  ${'team_'.$team_b_id.'_score'}=$match_details->scores->{$team_b_id.'_score'}.' sets';
	...
	$serving_array=Helper::getvolleyballServer($match_id, 'kabaddi');		
	?>
                        ...
                        <div class='col-sm-6 col-xs-12'>
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    <tbody id="team_tr_a" >
                                    @foreach($team_a_kabaddi_scores_array as $player_a)
                        ...
                        <div class='col-sm-6 col-xs-12'>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    <tbody id="team_tr_b" >
                                    @foreach($team_b_kabaddi_scores_array  as $player_b)
                        ...
                        <div class='col-sm-6 col-xs-12'>
                            <div class="table-responsive">
                                <table class="table ">
                                    ...
                                    <tbody id="team_tr_a" >
                                    @foreach($team_a_kabaddi_scores_array  as $k=>$player_a)
                                        @if($player_a['playing_status']=='P')
                                            <tr class="team_a_playing_row  ">
                                                <td>
                        ...
                        <div class='col-sm-6 col-xs-12'>
                            ...
                            <div class="table-responsive">
                                <table class="table ">
                                    ...
                                    <tbody id="team_tr_b" >
                                    @foreach($team_b_kabaddi_scores_array  as $k=>$player_b)
                                        @if($player_b['playing_status']=='P' )
                                            <tr class="team_a_playing_row">
                                                <td>
                                                 <div class='col-xs-2'>  {{ $k +1 }} </div>
      ...
      <table class='table table-striped table-bordered'>
        ...
        <tbody>
          <tr>
            ...
            <td>{{$kabaddi_a_score['team_name']}}</td>
            
          @for($set_index=1; $set_index<=$set; $set_index++)
            <td>
               
                 <input  readonly class="gui-input validation allownumericwithdecimal tennis_input_new a_set{{$set_index}}" value="{{$kabaddi_a_score['set'.$set_index]}}" name='a_set{{$set_index}}'>
          ...
          <tr>
            <td>{{$kabaddi_b_score['team_name']}} </td>

            @for($set_index=1; $set_index<=$set; $set_index++)
              <td>
               
                  <input  readonly class="gui-input validation allownumericwithdecimal tennis_input_new b_set{{$set_index}}" value="{{$kabaddi_b_score['set'.$set_index]}}" name='b_set{{$set_index}}'>
	...
	<script>
		...
		function confirmSquad(){
			...
			if(playing_players_a!=active_players){
					$.alert({
						title:"Alert",
						content:"Choose " + active_players + " players for Team A"
					})
			...
			if(playing_players_b!=active_players){
					$.alert({
						title:"Alert",
						content:"Choose " +active_players+ " players for Team B"
					})
			...
			if(substitute_players_a>max_substitutes){
					$.alert({
						title:"Alert",
						content:"You can only select a maximum of " + max_substitutes+ " substitutes for Team A"
					})
...
<script type="text/javascript">
	...
	function submitServingPlayers(that){
		...
		var p_arrays=[];
			...
			$.ajax({
				url:'/match/submitServingPlayerskabaddi',
				type:'post',
				data:data,
				success:function(response){
					window.location=window.location;
