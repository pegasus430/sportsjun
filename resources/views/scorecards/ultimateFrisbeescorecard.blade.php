...
<!-- show alert for no results -->
   ...
   <form id='ultimateFrisbee' onsubmit='return manualScoring(this)'>
   				{!!csrf_field()!!}
   					 <div class="row">
					    <div class='col-sm-12'>
					    <span>
					...
					<div id='real_time_scoring'  class="col-sm-12">
						...
						{{--
							<label class="col-sm-3 col-xs-5">
								...
								@for($i=1; $i<=$number_of_quarters; $i++)
									<option value="quarter_{{$i}}">Half {{$i}}</option>
								@endfor
								</select>
							</label>
						--}}
							...
							<button data-toggle="modal" data-target="#ultimateFrisbeeSubstituteModalB" class='btn-link btn-secondary-link  request pull-right' onclick="return false">Substitute  B</button>
						</center>
						<div class='row'>
							<p><br>
						</div>
									...
									<div class="row">
										...
										<div class="form-group scorescard_stats">
											...
											<select name="player_of_the_match" id="player_of_the_match" class=" form-control " onchange="">
												...
												@foreach($team_a_ultimateFrisbee_scores_array as $tm_player)
												...
												@foreach($team_b_ultimateFrisbee_scores_array as $tm_player)
													<option value="{{$tm_player['user_id']}}" @if($match_data[0]['player_of_the_match']==$tm_player['user_id'])?'selected':'' @endif >{{$tm_player['player_name']}}</option>
												@endforeach
											</select>
										</div>
...
<!--********* MATCH REPORT End **************!-->
	...
	<input type="hidden" id="ultimateFrisbee_form_data" value="">
	<input type="hidden" name="team_a_count" value="{{ (count($team_a_ultimateFrisbee_scores_array)>0)?count($team_a_ultimateFrisbee_scores_array):1 }}" id="team_a_count">
	<input type="hidden" name="team_b_count" value="{{ (count($team_b_ultimateFrisbee_scores_array)>0)?count($team_b_ultimateFrisbee_scores_array):1 }}" id="team_b_count">
			...
			<ul class="list-inline sportsjun-forms">
				@if($isValidUser)
					<li>
						...
						@if(!$match_data[0]['hasSetupSquad'])
							<button type='button' class='btn-danger btn .' onclick="$('#ultimateFrisbeePreferences').modal('show')" ><i class="fa fa-floppy-o"></i> Confirm Squad</button>
						@else
	...
	<div id="ultimateFrisbeeSubstituteModalA" class="modal fade">
		<div class="modal-dialog sj_modal sportsjun-forms">
			<div class="modal-content">
				<div class="alert alert-danger" id="div_failure1"></div>
				<div class="alert alert-success" id="div_success1" style="display:none;"></div>
				<div class="modal-body">
					<form  onsubmit="return ultimateFrisbeeSwapPlayers('form_substitute_a')" id='form_substitute_a'>
						{!!csrf_field()!!}
							...
							<div class="table-responsive">
								...
								<div class='col-sm-6'>
									...
									<table class="table ">
										...
										<tbody id="">
										@foreach($team_a_ultimateFrisbee_scores_array as $key=>$team_a_ultimateFrisbee)
											@if($team_a_ultimateFrisbee['playing_status']=="P" )
												<tr class="player_details_{{$team_a_ultimateFrisbee['id']}}">
													<td colspan="3" class='select_left' class="team_a_goal_row player_select " player_id="{{$team_a_ultimateFrisbee['id']}} "  player_name="{{$team_a_ultimateFrisbee['player_name']}}" team_name="{{$team_a_name}}" >       {{$team_a_ultimateFrisbee['player_name']}}
														{!! $team_a_ultimateFrisbee['has_substituted']==1?"<i class='fa fa-share'></i>":''!!}
													</td>
													<td colspan="1" >
														<input type='checkbox' name="substitute_a_{{$team_a_ultimateFrisbee['id']}}"  >
													</td>
												</tr>
											@endif
										@endforeach
								...
								<div class='col-sm-6'>
									...
									<table class="table table-striped">
										...
										<tbody id="">
										@foreach($team_a_ultimateFrisbee_scores_array as $key=>$team_a_ultimateFrisbee)
											@if($team_a_ultimateFrisbee['playing_status']=="S" )

												<tr class="player_details_{{$team_a_ultimateFrisbee['id']}}">
													<td colspan="3" class='select_left' class="team_a_goal_row player_select" player_id="{{$team_a_ultimateFrisbee['id']}} "  player_name="{{$team_a_ultimateFrisbee['player_name']}}" team_name="{{$team_a_name}}" >       {{$team_a_ultimateFrisbee['player_name']}}
														{!!$team_a_ultimateFrisbee['has_substituted']==1?"<i class='fa fa-reply'></i> {$team_a_ultimateFrisbee['time_substituted']} \"":'' !!}
	...
	<div id="ultimateFrisbeeSubstituteModalB" class="modal fade">
		<div class="modal-dialog sj_modal sportsjun-forms">
			<div class="modal-content">
				<div class="alert alert-danger" id="div_failure1"></div>
				<div class="alert alert-success" id="div_success1" style="display:none;"></div>
				<div class="modal-body">
					<div class='row'>
						<form  onsubmit="return ultimateFrisbeeSwapPlayers('form_substitute_b')" id='form_substitute_b'>
							{!!csrf_field()!!}
							<div class="table-responsive">
								...
								<div class='col-sm-6'>
									...
									<table class="table ">
										...
										<tbody id="">
										@foreach($team_b_ultimateFrisbee_scores_array as $team_a_ultimateFrisbee)
											@if($team_a_ultimateFrisbee['playing_status']=="P" )
												<tr class="player_details_{{$team_a_ultimateFrisbee['id']}}">
													<td colspan="3" class='select_left' class="team_a_goal_row player_select" player_id="{{$team_a_ultimateFrisbee['id']}} "  player_name="{{$team_a_ultimateFrisbee['player_name']}}" team_name="{{$team_a_name}}" >       {{$team_a_ultimateFrisbee['player_name']}}
														{!! $team_a_ultimateFrisbee['has_substituted']==1?"<i class='fa fa-share'></i> {$team_a_ultimateFrisbee['time_substituted']}\"":'' !!}
													</td>
													<td colspan="1" >
														<input type='checkbox' name="substitute_a_{{$team_a_ultimateFrisbee['id']}}" >
													</td>
									...
									<table class="table table-striped">
										...
										@foreach($team_b_ultimateFrisbee_scores_array as $team_a_ultimateFrisbee)
											@if($team_a_ultimateFrisbee['playing_status']=="S" )
												<tr class="player_details_{{$team_a_ultimateFrisbee['id']}}">
													<td colspan="3" class='select_left' class="team_a_goal_row player_select" player_id="{{$team_a_ultimateFrisbee['id']}} "  player_name="{{$team_a_ultimateFrisbee['player_name']}}" team_name="{{$team_a_name}}" >       {{$team_a_ultimateFrisbee['player_name']}}
														{!! $team_a_ultimateFrisbee['has_substituted']==1?"<i class='fa fa-reply'></i>":'' !!}
													</td>
													<td colspan="1" >
														<input type='checkbox' name="substitute_a_{{$team_a_ultimateFrisbee['id']}}" >
													</td>
	...
	<div id="ultimateFrisbeePreferences" class="modal fade">
		<div class="modal-dialog sj_modal sportsjun-forms">
			<div class="modal-content">
				<div class="alert alert-danger" id="div_failure1"></div>
				<div class="alert alert-success" id="div_success1" style="display:none;"></div>
...
<input type='hidden' id='match_players' value="{{json_encode($match_players)}}">
	...
	<script>
		...
		var team_a_count='{{ (count($team_a_ultimateFrisbee_scores_array)>0)?count($team_a_ultimateFrisbee_scores_array):1 }}';
		var team_b_count='{{ (count($team_b_ultimateFrisbee_scores_array)>0)?count($team_b_ultimateFrisbee_scores_array):1 }}';
	...
	<script>
		...
		function confirmSquad(){
			...
			$.confirm({
				...
				confirm:function(){
					...
					$.ajax({
						url:base_url+'/match/confirmSquadultimateFrisbee',
						data:tempSquadData,
						type:'post',
						success:function(response){
		...
		function saveMatchDetails(){
			var data=$('#ultimateFrisbee').serialize();
			$.ajax({
				url:base_url+'/match/insertAndUpdateultimateFrisbeeCard',
				data:data,
				method:'post',
				success:function(response){
		...
		function saveRecord(i, record_type,player_id){
			...
			$.ajax({
				url:base_url+'/match/saveMatchRecordultimateFrisbee',
				data:data,
				method:'post',
				dataType:'json',
				success:function(response){
					//setTimeout(getultimateFrisbeeDetails,2000);
		...
		function getultimateFrisbeeDetails(){
			//load details
				var data={
					match_id:$('#match_id').val(),
					team_a_id:{{$team_a_id}},
					...
					$.ajax({
						url:base_url+'/match/getultimateFrisbeeDetails',
						method:'get',
						data:data,
						success:function(response){
		...
		function ultimateFrisbeeSwapPlayers(ser_id){
			var data=$('#'+ser_id).serialize();
			$.ajax({
				url:base_url+'/match/ultimateFrisbeeSwapPlayers',
				data:data,
				method:'post',
				success:function(response){
					window.location=window.location;
	...
	<script type="text/javascript">
		...
		function ultimateFrisbeeChoosePenaltiesPlayers(form_id){
			$('#display_penalty_players').show();
			var data=$('#'+form_id).serialize();
			$.ajax({
				url:base_url+'/match/choosePenaltyPlayersultimateFrisbee',
				data:data,
				method:'post',
				dataType:'json',
				success:function(response){
					...
					$('#ultimateFrisbeePenaltiesModal').modal('hide');
		...
		function scorePenalty(that){
			...
			$.ajax({
				url:base_url+'/match/scorePenaltyultimateFrisbee',
				type:'post',
				data:data,
				dataType:'json',
				success:function(response){
...
var manual=false;
     ...
     function manualScoring(that){
        ...
        $.ajax({
            url:base_url+"/match/manualScoringultimateFrisbee",
            type:'post',
            data:data,
            success:function(response){
                window.location=window.location;
        ...
        $.ajax({
            url:base_url+"/match/endMatchRecordultimateFrisbee",
            type:'post', 
            data:data,
            success:function(response){
                window.location=window.location;
