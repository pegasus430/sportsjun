...
<!-- show alert for no results -->
   ...
   <form id='waterpolo' onsubmit='return manualScoring(this)'>
   				{!!csrf_field()!!}
   					 <div class="row">
					    <div class='col-sm-12'>
					    <span>
      ...
      <input type='text' class="tennis_input_new gui-input  points_1_player_{{$player['user_id']}}" readonly="" name="points_1_{{$player['id']}}" value="{{$player['points_1']}}" >
					...
					<div id='real_time_scoring'  class="col-sm-12">
						...
						<center class=" sportsjun-forms "  >
							<button href="javascript:void(0);" data-toggle="modal" data-target="#waterpoloSubstituteModalA" class='btn-link btn-other btn-secondary-link  request pull-left' onclick="return false">Substitute A</button>
...
<!-- Temporal Data -->
									...
									<div class="row">
										...
										<div class="form-group scorescard_stats">
											...
											<select name="player_of_the_match" id="player_of_the_match" class=" form-control " onchange="">
												...
												@foreach($team_b_waterpolo_scores_array as $tm_player)
													<option value="{{$tm_player['user_id']}}" @if($match_data[0]['player_of_the_match']==$tm_player['user_id'])?'selected':'' @endif >{{$tm_player['player_name']}}</option>
												@endforeach
											</select>
												</div>
	...
	<input type="hidden" id="waterpolo_form_data" value="">
	<input type="hidden" name="team_a_count" value="{{ (count($team_a_waterpolo_scores_array)>0)?count($team_a_waterpolo_scores_array):1 }}" id="team_a_count">
	<input type="hidden" name="team_b_count" value="{{ (count($team_b_waterpolo_scores_array)>0)?count($team_b_waterpolo_scores_array):1 }}" id="team_b_count">
			...
			<ul class="list-inline sportsjun-forms">
				@if($isValidUser)
					<li>
						...
						@if(!$match_data[0]['hasSetupSquad'])
							<button type='button' class='btn-danger btn .' onclick="$('#waterpoloPreferences').modal('show')" ><i class="fa fa-floppy-o"></i> Confirm Squad</button>
						@else
	...
	<div id="waterpoloSubstituteModalA" class="modal fade">
		<div class="modal-dialog sj_modal sportsjun-forms">
			<div class="modal-content">
				<div class="alert alert-danger" id="div_failure1"></div>
				<div class="alert alert-success" id="div_success1" style="display:none;"></div>
				<div class="modal-body">
					<form  onsubmit="return waterpoloSwapPlayers('form_substitute_a')" id='form_substitute_a'>
						{!!csrf_field()!!}
							...
							<div class="table-responsive">
								...
								<div class='col-sm-6'>
									...
									<table class="table ">
										...
										<tbody id="">
										@foreach($team_a_waterpolo_scores_array as $key=>$team_a_waterpolo)
											@if($team_a_waterpolo['playing_status']=="P" )
												<tr class="player_details_{{$team_a_waterpolo['id']}}">
													<td colspan="3" class='select_left' class="team_a_goal_row player_select " player_id="{{$team_a_waterpolo['id']}} "  player_name="{{$team_a_waterpolo['player_name']}}" team_name="{{$team_a_name}}" >       {{$team_a_waterpolo['player_name']}}
														{!! $team_a_waterpolo['has_substituted']==1?"<i class='fa fa-share'></i>":''!!}
													</td>
													<td colspan="1" >
														<input type='checkbox' name="substitute_a_{{$team_a_waterpolo['id']}}"  >
													</td>
												</tr>
											@endif
										@endforeach
								...
								<div class='col-sm-6'>
									...
									<table class="table table-striped">
										...
										<tbody id="">
										@foreach($team_a_waterpolo_scores_array as $key=>$team_a_waterpolo)
											@if($team_a_waterpolo['playing_status']=="S" )

												<tr class="player_details_{{$team_a_waterpolo['id']}}">
													<td colspan="3" class='select_left' class="team_a_goal_row player_select" player_id="{{$team_a_waterpolo['id']}} "  player_name="{{$team_a_waterpolo['player_name']}}" team_name="{{$team_a_name}}" >       {{$team_a_waterpolo['player_name']}}
														{!!$team_a_waterpolo['has_substituted']==1?"<i class='fa fa-reply'></i> {$team_a_waterpolo['time_substituted']} \"":'' !!}
	...
	<div id="waterpoloSubstituteModalB" class="modal fade">
		<div class="modal-dialog sj_modal sportsjun-forms">
			<div class="modal-content">
				<div class="alert alert-danger" id="div_failure1"></div>
				<div class="alert alert-success" id="div_success1" style="display:none;"></div>
				<div class="modal-body">
					<div class='row'>
						<form  onsubmit="return waterpoloSwapPlayers('form_substitute_b')" id='form_substitute_b'>
							{!!csrf_field()!!}
							<div class="table-responsive">
								...
								<div class='col-sm-6'>
									...
									<table class="table ">
										...
										<tbody id="">
										@foreach($team_b_waterpolo_scores_array as $team_a_waterpolo)
											@if($team_a_waterpolo['playing_status']=="P" )
												<tr class="player_details_{{$team_a_waterpolo['id']}}">
													<td colspan="3" class='select_left' class="team_a_goal_row player_select" player_id="{{$team_a_waterpolo['id']}} "  player_name="{{$team_a_waterpolo['player_name']}}" team_name="{{$team_a_name}}" >       {{$team_a_waterpolo['player_name']}}
														{!! $team_a_waterpolo['has_substituted']==1?"<i class='fa fa-share'></i> {$team_a_waterpolo['time_substituted']}\"":'' !!}
													</td>
													<td colspan="1" >
														<input type='checkbox' name="substitute_a_{{$team_a_waterpolo['id']}}" >
													</td>
									...
									<table class="table table-striped">
										...
										@foreach($team_b_waterpolo_scores_array as $team_a_waterpolo)
											@if($team_a_waterpolo['playing_status']=="S" )
												<tr class="player_details_{{$team_a_waterpolo['id']}}">
													<td colspan="3" class='select_left' class="team_a_goal_row player_select" player_id="{{$team_a_waterpolo['id']}} "  player_name="{{$team_a_waterpolo['player_name']}}" team_name="{{$team_a_name}}" >       {{$team_a_waterpolo['player_name']}}
														{!! $team_a_waterpolo['has_substituted']==1?"<i class='fa fa-reply'></i>":'' !!}
													</td>
													<td colspan="1" >
														<input type='checkbox' name="substitute_a_{{$team_a_waterpolo['id']}}" >
													</td>
	...
	<div id="waterpoloPreferences" class="modal fade">
		<div class="modal-dialog sj_modal sportsjun-forms">
			<div class="modal-content">
				<div class="alert alert-danger" id="div_failure1"></div>
				<div class="alert alert-success" id="div_success1" style="display:none;"></div>
...
<input type='hidden' id='match_players' value="{{json_encode($match_players)}}">
	...
	<script>
		...
		function confirmSquad(){
			...
			$.confirm({
				...
				confirm:function(){
					...
					$.ajax({
						url:base_url+'/match/confirmSquadwaterpolo',
						data:tempSquadData,
						type:'post',
						success:function(response){
		...
		function saveMatchDetails(){
			var data=$('#waterpolo').serialize();
			$.ajax({
				url:base_url+'/match/insertAndUpdatewaterpoloCard',
				data:data,
				method:'post',
				success:function(response){
		...
		function saveRecord(i, record_type,player_id){
			...
			$.ajax({
				url:base_url+'/match/saveMatchRecordwaterpolo',
				data:data,
				method:'post',
				dataType:'json',
				success:function(response){
					//setTimeout(getwaterpoloDetails,2000);
		...
		function getwaterpoloDetails(){
			//load details
				var data={
					...
					$.ajax({
						url:base_url+'/match/getwaterpoloDetails',
						method:'get',
						data:data,
						success:function(response){
		...
		function waterpoloSwapPlayers(ser_id){
			var data=$('#'+ser_id).serialize();
			$.ajax({
				url:base_url+'/match/waterpoloSwapPlayers',
				data:data,
				method:'post',
				success:function(response){
					window.location=window.location;
	...
	<script type="text/javascript">
		...
		function waterpoloChoosePenaltiesPlayers(form_id){
			$('#display_penalty_players').show();
			var data=$('#'+form_id).serialize();
			$.ajax({
				url:base_url+'/match/choosePenaltyPlayerswaterpolo',
				data:data,
				method:'post',
				dataType:'json',
				success:function(response){
					...
					$('#waterpoloPenaltiesModal').modal('hide');
		...
		function scorePenalty(that){
			...
			$.ajax({
				url:base_url+'/match/scorePenaltywaterpolo',
				type:'post',
				data:data,
				dataType:'json',
				success:function(response){
...
var manual=false;
     ...
     function manualScoring(that){
        ...
        $.ajax({
            url:base_url+"/match/manualScoringwaterpolo",
            type:'post',
            data:data,
            success:function(response){
                window.location=window.location;
        ...
        $.ajax({
            url:base_url+"/match/endMatchRecordwaterpolo",
            type:'post', 
            data:data,
            success:function(response){
                window.location=window.location;
