  ...
  ${'team_'.$team_b_id.'_score'}=$match_details->scores->{$team_b_id.'_score'}.' sets';
	...
	<div class="col_standard soccer_scorecard">
		...
		<div class="container pull-up">
			...
			<div class="row">
				<div class="col-md-12">
					<h5 class="scoreboard_title">throwball Scorecard
                        ...
                        <div class='col-sm-6 col-xs-12'>
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    <tbody id="team_tr_a" >
                                    @foreach($team_a_throwball_scores_array as $player_a)
                        ...
                        <div class='col-sm-6 col-xs-12'>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    <tbody id="team_tr_b" >
                                    @foreach($team_b_throwball_scores_array  as $player_b)
                        ...
                        <div class='col-sm-6 col-xs-12'>
                            <div class="table-responsive">
                                <table class="table ">
                                    ...
                                    <tbody id="team_tr_a" >
                                    @foreach($team_a_throwball_scores_array  as $k=>$player_a)
                                        @if($player_a['playing_status']=='P')
                                            <tr class="team_a_playing_row  ">
                                                <td>
                        ...
                        <div class='col-sm-6 col-xs-12'>
                            ...
                            <div class="table-responsive">
                                <table class="table ">
                                    ...
                                    <tbody id="team_tr_b" >
                                    @foreach($team_b_throwball_scores_array  as $k=>$player_b)
                                        @if($player_b['playing_status']=='P' )
                                            <tr class="team_a_playing_row">
                                                <td>
                                                 <div class='col-xs-2'>  {{ $k +1 }} </div>
...
<!-- show alert for no results -->
   ...
   <form id='throwball' onsubmit='return manualScoring(this)'>
   				{!!csrf_field()!!}
   					 <div class="row">
					    <div class='col-sm-12'>
      ...
      <table class='table table-striped table-bordered'>
        ...
        <tbody>
          <tr>
            ...
            <td>{{$throwball_a_score['team_name']}}</td>
            
          @for($set_index=1; $set_index<=$set; $set_index++)
            <td>
               
                 <input  readonly class="gui-input validation allownumericwithdecimal tennis_input_new a_set{{$set_index}}" value="{{$throwball_a_score['set'.$set_index]}}" name='a_set{{$set_index}}'>
          ...
          <tr>
            <td>{{$throwball_b_score['team_name']}} </td>

            @for($set_index=1; $set_index<=$set; $set_index++)
              <td>
               
                  <input  readonly class="gui-input validation allownumericwithdecimal tennis_input_new b_set{{$set_index}}" value="{{$throwball_b_score['set'.$set_index]}}" name='b_set{{$set_index}}'>
            ...
            @endfor
					...
					<div id='real_time_scoring'  class="col-sm-12">
						...
						<div class='row'>
							...
							<button href="javascript:void(0);" data-toggle="modal" data-target="#throwballSubstituteModalA" class='btn-link btn-other btn-secondary-link  request pull-left' onclick="return false">Substitute A</button>
						...
						<div class="sportsjun-forms">						
							...
							<button data-toggle="modal" data-target="#throwballSubstituteModalB" class='btn-link btn-secondary-link  request pull-right' onclick="return false">Substitute  B</button>
						</div>
									...
									<div class="col-sm-4">
										...
										<div class="form-group scorescard_stats">
											...
											<select name="player_of_the_match" id="player_of_the_match" class=" form-control " onchange="">
												...
												@foreach($team_a_throwball_scores_array as $tm_player)
												...
												@foreach($team_b_throwball_scores_array as $tm_player)
													<option value="{{$tm_player['user_id']}}" @if($match_data[0]['player_of_the_match']==$tm_player['user_id'])?'selected':'' @endif >{{$tm_player['player_name']}}</option>
												@endforeach
											</select>
										</div>
...
<!--********* MATCH REPORT End **************!-->
	...
	<input type="hidden" id="throwball_form_data" value="">
	<input type="hidden" name="team_a_count" value="{{ (count($team_a_throwball_scores_array)>0)?count($team_a_throwball_scores_array):1 }}" id="team_a_count">
	<input type="hidden" name="team_b_count" value="{{ (count($team_b_throwball_scores_array)>0)?count($team_b_throwball_scores_array):1 }}" id="team_b_count">
	...
	<div id="throwballSubstituteModalA" class="modal fade">
		<div class="modal-dialog sj_modal sportsjun-forms">
			<div class="modal-content">
				<div class="alert alert-danger" id="div_failure1"></div>
				<div class="alert alert-success" id="div_success1" style="display:none;"></div>
				<div class="modal-body">
					<form  onsubmit="return throwballSwapPlayers('form_substitute_a')" id='form_substitute_a'>
						{!!csrf_field()!!}
						<input type='hidden' name='match_id' value="{{$match_id}}">
							...
							<div class="table-responsive">
								...
								<div class='col-sm-6'>
									...
									<table class="table ">
										...
										<tbody id="">
										@foreach($team_a_throwball_scores_array as $key=>$team_a_throwball)
											@if($team_a_throwball['playing_status']=="P" )
												<tr class="player_details_{{$team_a_throwball['id']}}">
													<td colspan="3" class='select_left' class="team_a_goal_row player_select " player_id="{{$team_a_throwball['id']}} "  player_name="{{$team_a_throwball['player_name']}}" team_name="{{$team_a_name}}" >       {{$team_a_throwball['player_name']}}
													
													</td>
													<td colspan="1" >
														<input type='checkbox' name="substitute_a_{{$team_a_throwball['id']}}"  >
													</td>
												</tr>
											@endif
										@endforeach
								...
								<div class='col-sm-6'>
									...
									<table class="table table-striped">
										...
										<tbody id="">
										@foreach($team_a_throwball_scores_array as $key=>$team_a_throwball)
											@if($team_a_throwball['playing_status']=="S" )
	...
	<div id="throwballSubstituteModalB" class="modal fade">
		<div class="modal-dialog sj_modal sportsjun-forms">
			<div class="modal-content">
				<div class="alert alert-danger" id="div_failure1"></div>
				<div class="alert alert-success" id="div_success1" style="display:none;"></div>
				<div class="modal-body">
					<div class='row'>
						<form  onsubmit="return throwballSwapPlayers('form_substitute_b')" id='form_substitute_b'>
							{!!csrf_field()!!}
							<div class="table-responsive">
								...
								<div class='col-sm-6'>
									...
									<table class="table ">
										...
										<tbody id="">
										@foreach($team_b_throwball_scores_array as $team_a_throwball)
											@if($team_a_throwball['playing_status']=="P" )
												<tr class="player_details_{{$team_a_throwball['id']}}">
													<td colspan="3" class='select_left' class="team_a_goal_row player_select" player_id="{{$team_a_throwball['id']}} "  player_name="{{$team_a_throwball['player_name']}}" team_name="{{$team_a_name}}" >       {{$team_a_throwball['player_name']}}
...
$(document).ready(function(){
		...
		var team_a_count='{{ (count($team_a_throwball_scores_array)>0)?count($team_a_throwball_scores_array):1 }}';
		var team_b_count='{{ (count($team_b_throwball_scores_array)>0)?count($team_b_throwball_scores_array):1 }}';
	...
	<script>
		...
		function confirmSquad(){
			...
			$.confirm({
				...
				confirm:function(){
					...
					$.ajax({
						url:base_url+'/match/confirmSquadthrowball',
						data:tempSquadData,
						type:'post',
						success:function(response){
		...
		function saveMatchDetails(){
			var data=$('#throwball').serialize();
			$.ajax({
				url:base_url+'/match/insertAndUpdatethrowballCard',
				data:data,
				method:'post',
				success:function(response){
		...
		function getthrowballDetails(){
			//load details
				var data={
					...
					$.ajax({
						url:base_url+'/match/getthrowballDetails',
						method:'get',
						data:data,
						success:function(response){
		...
		function throwballSwapPlayers(ser_id){
			var data=$('#'+ser_id).serialize();
			$.ajax({
				url:base_url+'/match/throwballSwapPlayers',
				data:data,
				method:'post',
				success:function(response){					
					window.location=window.location;
...
var manual=false;
     ...
     function manualScoring(that){
        ...
        $.ajax({
            url:base_url+"/match/manualScoringthrowball",
            type:'post',
            data:data,
            success:function(response){
                window.location=window.location;
        ...
        $.ajax({
            url:base_url+"/match/endMatchRecordthrowball",
            type:'post', 
            data:data,
            success:function(response){
                window.location=window.location;
...
<script type="text/javascript">
	...
	function submitServingPlayers(that){
		...
		var p_arrays=[];
			...
			$.ajax({
				url:'/match/submitServingPlayersthrowball',
				type:'post',
				data:data,
				success:function(response){
					window.location=window.location;
